...
describe('simpleCalculator tests', () => {
  ...
  test('should subtract two numbers', () => {
    ...
    expect(simpleCalculator({ a: 102, b: -33, action: Action.Subtract })).toBe(
      135,
    );
  });
  ...
  test('should multiply two numbers', () => {
    ...
    expect(simpleCalculator({ a: 12, b: -3, action: Action.Multiply })).toBe(
      -36,
    );
  });
  ...
  test('should return null for invalid arguments', () => {
    expect(
      simpleCalculator({ a: 'ab', b: 0, action: Action.Exponentiate }),
    ).toBe(null);
    expect(simpleCalculator({ a: 'ab', b: 'cd', action: Action.Add })).toBe(
      null,
    );
    expect(simpleCalculator({ a: 1, b: 'cd', action: Action.Subtract })).toBe(
      null,
    );
  });
});
